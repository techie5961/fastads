 ...
 //   return 'tech';
  $transactions=DB::table('transactions')->where('user_id',Auth::guard('users')->user()->id)->orderBy('date','desc')->paginate(5);
    ...
    public function PostAds(){
        $ads_category=DB::table('ads_category')->orderBy('cost','asc')->get();
        return view('users.ads.post',[
            'categories' => $ads_category
        ]);
    }
    // posted ads
    public function PostedAds(){
        $ads=DB::table('tasks')->orderBy('date','desc')->paginate(10);
        $ads->getCollection()->transform(function($each){
    $each->frame=Carbon::parse($each->date)->diffForHumans();
    $proofs=DB::table('task_proofs')->where('task_id',$each->id)->count();
    $each->status=$proofs >= $each->limit ? 'completed' : 'in progress';
    return $each;
        });
        return view('users.ads.manage',[
            'ads' => $ads
        ]);
    }
    // recharge
    ...
    public function Invite(){
        return view('users.invite',[
            ...
            'referred' => DB::table('users')->where('ref',Auth::guard('users')->user()->username)->count(),

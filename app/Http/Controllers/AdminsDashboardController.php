...
class AdminsDashboardController extends Controller
{
    ...
    public function ManageTasks(){
        ...
        $tasks->getCollection()->transform(function($each){
            ...
            $each->cat=json_decode($each->category);
          
            $each->category=DB::table('categories')->where('id',$each->cat->id)->first() ?? $each->cat;
    ...
    public function EditTask(){
        ...
        $data->subcategory=json_decode($data->subcategory);
        $subcategories=DB::table('subcategories')->where('category->id',$data->category->id)->get();
    ...
    public function TasksSubmitted(){
        ...
        if(request()->has('task_id')){
            $proofs=DB::table('task_proofs')->where('json->id',request('task_id'))->orderBy('date','desc')->paginate(10);
        }
    ...
    public function ManageCategories(){
        $categories=DB::table('categories')->orderBy('date','desc')->paginate(10);
        ...
        return view('admins.categories.manage',[
            'total' => DB::table('categories')->count(),
            'active' => DB::table('categories')->where('status','active')->count(),
            'today' => DB::table('categories')->whereDate('date',Carbon::today())->count(),
            'categories' => $categories
        ]);
    }
     // add subcategories
    public function AddSubCategory(){
        ...
        return view('admins.subcategories.add',[
            'categories' => DB::table('categories')->orderBy('name','asc')->get()
        ]);
    }
    // edit subcategories
    public function EditSubCategory(){
        ...
        $data->category=json_decode($data->category);
        return view('admins.subcategories.edit',[
            'subcategory' => $data,
            'categories' => DB::table('categories')->orderBy('name','asc')->get()
        ]);
    }
    ...
    public function ManageSubCategories(){
        $subcategories=DB::table('subcategories')->orderBy('date','desc')->paginate(10);
        $subcategories->getCollection()->transform(function($each){
            $each->frame=Carbon::parse($each->date)->diffForHumans();
        ...
        return view('admins.subcategories.manage',[
            'total' => DB::table('subcategories')->count(),
            'active' => DB::table('subcategories')->where('status','active')->count(),
            'today' => DB::table('subcategories')->whereDate('date',Carbon::today())->count(),
            'subcategories' => $subcategories
        ]);
    }
    // add category
    public function AddAdsCategory(){
    return view('admins.ads.categories.add');
    }
    // manage ads categories
    public function ManageAdsCategories(){
        $categories=DB::table('ads_category')->paginate(10);
        $categories->getCollection()->transform(function($each){
    $each->frame=Carbon::parse($each->updated);
    return $each;
        });
        return view('admins.ads.categories.manage',[
            'total' => DB::table('ads_category')->count(),
            'categories' => $categories
        ]);
    }
}

...
class UserPostRequestController extends Controller
{
       ...
       if(request()->input('ref') !== ''){
            ...
            //   direct
              DB::table('users')->where('id',$ref->id)->update([
                'affiliate_balance' => DB::raw('affiliate_balance + '.$general_settings->referral->first.'')
              ]);
            ...
            'class' => 'credit',
            'amount' => $general_settings->referral->first,
    ...
    public function Withdraw(){
     ...
     $finance=json_decode(DB::table('settings')->where('key','finance_settings')->first()->json ?? '{}');
      if(request()->input('amount') < $minimum_withdrawal){
        return response()->json([
            'message' => 'Minimum withdrawal for '.(request('wallet') == 'activities_balance' ? 'Activities' : 'Affiliate').' wallet is &#8358;'.number_format($minimum_withdrawal,2).'',
            'status' => 'error'
        ]);
      }
    ...
    public function ClaimTaskReward(){
        ...
        $task->reward=json_decode($task->category)->reward;
    ...
    public function PostAds(){
        $category=DB::table('ads_category')->where('id',request('category_id'))->first();
        if(Auth::guard('users')->user()->deposit_balance < $category->cost){
            return response()->json([
                'message' => 'Insufficient deposit balance. You need &#8358;'.number_format($category->cost,2).'',
                'status' => 'error'
            ]);
        }
        DB::table('users')->update([
            'deposit_balance' => DB::raw('deposit_balance - '.$category->cost.''),
         ...
         DB::table('transactions')->insert([
          ...
          DB::table('tasks')->insert([
            'title' => ucfirst(request('platform')).' Members',
        'uniqid' => Str::random(12),
        'category' => json_encode(DB::table('ads_category')->where('id',request('category_id'))->first()),
         'link' => request()->input('link'),
        'limit' => DB::table('ads_category')->where('id',request('category_id'))->first()->members,
        'updated' => Carbon::now(),
        'date' => Carbon::now()
       ]);
       return response()->json([
